...
# Fetch Data AFTER Sidebar renders
...
with st.sidebar:
    ...
    if c1.button("📈 CALL", use_container_width=True, disabled=(curr_bal < 10 or current_price == 0)):
        ...
        st.session_state.active_trades.append({"id": int(time.time()), "asset": asset_name, "type": "CALL", "amount": amount, "entry": current_price})
    if c2.button("📉 PUT", use_container_width=True, disabled=(curr_bal < 10 or current_price == 0)):
        st.session_state.balance -= amount
        st.session_state.active_trades.append({"id": int(time.time()), "asset": asset_name, "type": "PUT", "amount": amount, "entry": current_price})
